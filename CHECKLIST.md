...
- **`Or`-pattern alternatives binding a name, and guard-failure leaking a bound local — both fixed** (once the VM/conformance suite this note originally deferred to actually existed). The `Or`-pattern bug was broader than originally suspected here: not just repeated names across alternatives, but *any* `Or`-pattern where at least one alternative binds a name, since the compiler's `compile_pattern_match` never reset `local_count` between alternatives even though only one alternative's bind ever executes at runtime — confirmed as a genuine out-of-bounds VM stack panic, not just a wrong value. Fixed by having the resolver declare each distinct bound name exactly once (not once per occurrence) and having the compiler pre-reserve one physical slot per distinct name before any alternative's test runs, with every alternative writing into that reserved slot via a new parallel bind-compilation path — plus popping those reserved slots explicitly when every alternative's test fails, a leak an earlier draft of the fix missed and a subagent review caught. A second, independently-discovered, unrelated-to-`Or` bug was found and fixed alongside it while investigating: any guarded pattern that binds a name, whose guard can fail with another arm following, leaked the bound value onto the stack — confirmed to silently corrupt the *existing, previously-passing* `guard_and_or_pattern_both_work` test, which only happened to pass because the leaked value numerically equalled the correct answer. Both fixes share one root cause (a `fail_jumps` path bypassing `emit_tail_scope_exit`'s cleanup) and one technique (raw `Op::Pop` on the failure path, without touching the compiler's ongoing `local_count`/`declared_slots` bookkeeping, which the success path compiled immediately afterward still needs intact). See `docs/superpowers/specs/2026-08-09-or-pattern-shared-slot-fix-design.md`.
...
- **`let NAME = <expr with its own nested scope>` local-slot desync — found and fixed** (Phase 12, while writing conformance fixtures). The resolver declares a `let`'s own name *before* resolving its initializer (needed to reject `let x = x + 1;` as a self-reference), but the compiler declared it *after* compiling the initializer — so any nested scope inside the initializer (a block, an `if`/`else` branch) got resolver slot numbers one higher than where the compiler actually put them, causing either a VM panic or a silently wrong value (`let y = { let a = 1; let b = 2; a + b }; y;` printed `4`, not `3`). Fixed by reserving the name's physical slot (`Op::Nil` + a new `reserve_named_local`) *before* compiling the initializer, matching the resolver's own order, then writing the initializer's result into that reserved slot afterward. See `docs/superpowers/specs/2026-08-10-let-initializer-nested-scope-slot-desync-fix-design.md`.
...
- [x] 🔴 `tests/conformance/*.em` with paired `.expected` files — 14 fixtures (grew from a pre-existing 6).
- [x] 🔴 Harness runs each program on **both backends** and asserts byte-identical stdout
- [x] 🔴 Harness also asserts identical **error output** for failing programs — scoped to exact error-*message* identity (not byte-identical full diagnostics) for the 3 runtime error paths where both backends genuinely use the same format string (division by zero, integer overflow, stack overflow). The tree-walker's `RuntimeError` carries a full AST `Span`, while the VM's carries only a single bytecode-line-table-derived `line` (its own doc comment calls this "a byte-offset stand-in, not a real 1-based source line") — so full rendered-diagnostic parity isn't architecturally achievable today; every other runtime error message embeds a backend-specific `Value`'s `{:?}` output, which differs between the two `Value` enums, so those paths were excluded rather than papered over.
- [x] 🔴 Harness runs everything a third time under `gc-stress` — via a dedicated CI job (`cargo test -p ember-cli --features gc-stress --test conformance`), not baked into the same test function, since the feature already propagates through `ember-cli → ember-vm → ember-gc` with zero code changes.
- [x] 🔴 CI fails on any divergence — this single check is what validates the whole two-backend design
- [x] 🔴 Conformance programs covering: arithmetic, strings, lists, closures, recursion, ADTs+match, structs, generics, all loops, shadowing, higher-order functions, mutual recursion, deep recursion, error paths — `deep_recursion.em` uses `sum_to(50)`, not a larger depth, since the tree-walker enforces a deliberately low `MAX_CALL_DEPTH = 64` (`ember-tree/src/interp.rs`) to trip before the native Rust stack overflows; the point of the fixture is proving legal deep recursion works identically, not probing the overflow boundary (that's `stack_overflow.em`, under `tests/conformance_errors/`). Writing these fixtures surfaced and led to fixing a real VM miscompilation bug — see the note below.
- [x] 🔴 `tests/diagnostics/*.em` + `.stderr` snapshots — implemented as `insta` snapshots of the fully rendered diagnostic (`ember_diag::render::render`) rather than literal sidecar `.stderr` files; `insta`'s own snapshot/review workflow serves the identical "must not regress silently" purpose the checklist item is after.
- [x] 🔴 `insta` snapshots for AST and disassembly — the AST half was already satisfied by a pre-existing `ember-parser` suite (`snapshot_programs.rs`, 20 programs via `ember_ast::print_stmt`) predating this phase; this phase added the disassembly half (`ember_bytecode::disasm::disassemble_chunk` over all 14 conformance fixtures).
- [x] 🔴 Property test: parser round-trip — pre-existing, unchanged.
- [x] 🔴 Property test: lexer span tiling — pre-existing, unchanged.
- [x] 🟡 Property test: formatter idempotence over generated ASTs — uses whitespace/blank-line perturbations of the real conformance corpus rather than a from-scratch arbitrary-AST generator (a separate, large undertaking); still exercises the formatter's trickiest logic (comment/blank-line attachment) directly.
- [x] 🟡 Fuzz targets: lexer, parser, type checker (no panics) — real `cargo-fuzz`/libFuzzer targets (not workspace members, standard cargo-fuzz layout) plus a nightly-toolchain CI job running each for 30s on every push. No crashes found in initial smoke testing (~500K-780K executions per target in 10s each).
- [x] 🟡 `criterion` benchmarks: fib, loops, closures, list ops, string ops — both backends
- [x] 🟡 Allocation counting via a custom `GlobalAlloc` wrapper for the comparison panel — infrastructure only (`ember-vm`'s `CountingAlloc`, behind a `count-allocs` feature so default builds pay zero cost); CLI/playground surfacing is Phase 13/16's job.
- [x] 🟡 Benchmark regression gate in CI (>10% slowdown fails) — a checked-in comparison script (`scripts/check_bench_regression.py`) reading criterion's own `estimates.json` per baseline, since no off-the-shelf GitHub Action fit this repo's two-backend benchmark-group layout.
- [ ] 🟢 Coverage reporting — deferred.

**Design decisions and gaps beyond the checklist's literal scope, found or made during implementation:**
- **A real VM miscompilation bug was found and fixed while writing this phase's conformance fixtures**, unrelated to test infrastructure itself: `let NAME = <expr with its own nested scope, e.g. a block or if/else>` desynced the resolver's and compiler's local-slot numbering (the resolver declares a `let`'s name before resolving its initializer, to support rejecting `let x = x + 1;` as a self-reference; the compiler declared it *after* compiling the initializer), causing either a VM panic or — worse — a silently wrong value (`let y = { let a = 1; let b = 2; a + b }; y;` printed `4`, not `3`). Fixed by reserving the name's physical slot before compiling its initializer, matching the resolver's own order. See `docs/superpowers/specs/2026-08-10-let-initializer-nested-scope-slot-desync-fix-design.md` and the corresponding note in Phase 9's own design-decisions list above. This is exactly the kind of bug this phase's conformance suite exists to catch — and it was caught before the suite was even finished being built.
- **Fuzz targets are cargo-fuzz's own standalone layout, not workspace members** — deliberate, keeps the nightly-only libFuzzer toolchain dependency out of the stable workspace build entirely (`cargo build --workspace` on stable is unaffected).
